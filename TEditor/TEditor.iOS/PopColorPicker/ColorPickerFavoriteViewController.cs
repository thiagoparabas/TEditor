...
namespace PopColorPicker.iOS
{
    public class ColorPickerFavoriteViewController : UIViewController
    {
        ...
        public override void ViewDidLoad()
        {
            ...
            _paletteView = new UIView(new RectangleF(0, 10f, 320f, View.Frame.Height - 80f)) { BackgroundColor = UIColor.Clear };
            ...
            _deleteView = new UIView
            {
                ...
                Frame = new RectangleF(0f, View.Bounds.Bottom, View.Frame.Width, 190f)
            };
        ...
        private void SetColorGrid()
        {
            ...
            for (var i = 0; i < colorCount && i < _colors.Count; i++)
            {
                ...
                var layer = new CALayer
                {
                    Name = $"Color_{i}",
                    CornerRadius = 6f,
                    BackgroundColor = color.Value.CGColor,
                    Frame = new RectangleF(0f, 0f, 70f, 40f)
                };
                ...
                var myView = new UIView(frame) { Tag = 99 };
        ...
        public void ColorGridTapped(UITapGestureRecognizer recognizer)
        {
            ...
            if (touchedLayer != null && !string.IsNullOrWhiteSpace(touchedLayer.Name) && touchedLayer.Name.IndexOf("Color", StringComparison.Ordinal) == 0)
            {
            ...
            if (_previousLayer == null) return;
            var temp = _previousLayer.Name.Split(new[] { '_' });
            var index = int.Parse(temp[1]);


            if (index >= _colors.Count) return;
            var color = _colors.ElementAt(index);
            if (ParentViewController is PopColorPickerViewController parent) parent.SelectedColor = color.Value;
        }
    }
}
