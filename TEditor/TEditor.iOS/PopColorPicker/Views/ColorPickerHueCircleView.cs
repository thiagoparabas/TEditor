...
namespace PopColorPicker.iOS
{
    public class ColorPickerHueCircleView : UIView
    {
        ...
        private void SetCrosshairView()
        {
            ...
            frame.X = Frame.Width - frame.Width - 1;
            frame.Y = (Frame.Height / 2) - (frame.Height / 2);
        ...
        {
            var size = new SizeF(Bounds.Size.Width, Bounds.Size.Height);
            ...
            const float angle = 2f * (float)Math.PI / sectors;

            for (var i = 0; i < sectors; i++)
            {
                var center = new PointF(size.Width / 2, size.Height / 2);
                var bezierPath = UIBezierPath.FromArc(center, radius, i * angle, (i + 1) * angle, true);
        ...
        {
            var size = new SizeF(Bounds.Size.Width - 40f, Bounds.Size.Height - 40f);
        ...
        {
            var outerRadius = (Frame.Width / 2) - 10;
            ...
            center.X = (float)x + (Frame.Width / 2);
            center.Y = (float)y + (Frame.Width / 2);
