...
namespace PopColorPicker.iOS
{
    public class ColorPickerCustomViewController : UIViewController
    {
        ...
        private void SetHueCircleView()
        {
            ...
            View.AddSubview(_hueCircleView);
        }
        ...
        private void SetSliders()
        {
            ...
            const float sliderX = 55f;

            _sliderR = new UISlider(new RectangleF(new PointF(sliderX, 270f), sliderSize)) { MaxValue = 255, MinValue = 0, Value = 255 };
            _sliderR.ValueChanged += Slider_ValueChanged;
            View.AddSubview(_sliderR);

            _sliderG = new UISlider(new RectangleF(new PointF(sliderX, 270f + y), sliderSize)) { MaxValue = 255, MinValue = 0 };
            _sliderG.ValueChanged += Slider_ValueChanged;
            View.AddSubview(_sliderG);

            _sliderB = new UISlider(new RectangleF(new PointF(sliderX, 270f + (y * 2)), sliderSize)) { MaxValue = 255, MinValue = 0 };
            _sliderB.ValueChanged += Slider_ValueChanged;
            View.AddSubview(_sliderB);

            const float labelX = 40f;
            var labelSize = new SizeF(20f, 20f);

            var labelR = new UILabel(new RectangleF(new PointF(labelX, _sliderR.Frame.Y), labelSize)) { Text = "R", TextColor = UIColor.White };
            View.AddSubview(labelR);

            var labelG = new UILabel(new RectangleF(new PointF(labelX, _sliderG.Frame.Y), labelSize)) { Text = "G", TextColor = UIColor.White };
            View.AddSubview(labelG);

            var labelB = new UILabel(new RectangleF(new PointF(labelX, _sliderB.Frame.Y), labelSize)) { Text = "B", TextColor = UIColor.White };
            View.AddSubview(labelB);

            var labelValueX = sliderSize.Width + 65f;
            var labelValueSize = new SizeF(40f, 20f);

            _labelRValue = new UILabel(new RectangleF(new PointF(labelValueX, _sliderR.Frame.Y), labelValueSize)) { Text = "255", TextColor = UIColor.White };
            View.AddSubview(_labelRValue);

            _labelGValue = new UILabel(new RectangleF(new PointF(labelValueX, _sliderG.Frame.Y), labelValueSize)) { Text = "0", TextColor = UIColor.White };
            View.AddSubview(_labelGValue);

            _labelBValue = new UILabel(new RectangleF(new PointF(labelValueX, _sliderB.Frame.Y), labelValueSize)) { Text = "0", TextColor = UIColor.White };
            View.AddSubview(_labelBValue);
        }
        ...
        private void UpdateLabels()
        {
            ...
            if (ParentViewController is PopColorPickerViewController parent) parent.SelectedColor = UIColor.FromRGB(_hsbView.Red, _hsbView.Green, _hsbView.Blue);
        }
